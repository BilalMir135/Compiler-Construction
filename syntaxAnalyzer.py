class SyntaxAnalyzer:
    ...
    def staticOrVoid(self):
        if self.ts[self.index][0] == 'Static':
            ...
            if self.AMList(): #215
                return True
        elif self.ts[self.index][0] == 'Override':
    ...
    def override(self):
        if self.ts[self.index][0] == 'Override':
    ...
    def allInit(self):
        if self.ts[self.index][0] == 'Assignment Operator':
            if self.assign(): #512
                print('Match allInit() => ',self.ts[self.index])
                self.index += 1  #doubt --i
        else:
            if self.ts[self.index][0] == [';',',',')']:
                return True
    ...
    def values(self):
        if self.ts[self.index][0] in ['Identifier','Integer Constant','Float Constant','Char Constant','String Constant', 'Logical Operator', '(', 'Increment/Decrement Opeartor']: #Not !
            if self.init(): #798
                return True
        elif self.ts[self.index][0] == 'New':
